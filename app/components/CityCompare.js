...
const citySlug=n=>n.toLowerCase().replaceAll(' ','-').replace('new-york-city','new-york');
...
function CityCard({r,salary,debt,car,family,best}){const {d}=r;return <article className={`citycard procard ${best?'bestcard':''}`}><div className={`compare-city-photo city-${citySlug(r.city)}`}><span>{r.city}</span></div><div className="cityhead"><div><span>{d.country==='CA'?'🇨🇦':'🇺🇸'} CITY {best?'· HIGHEST MONEY LEFT':''}</span><h3>{r.city}</h3></div><b>{d.currency}</b></div><div className="cityhero"><span>Money left after modeled costs</span><strong className={r.left<0?'negative':''}>{fmt(r.left,d.currency)}</strong><small>/ month</small></div><div className="costrows"><p><span>Gross salary</span><b>{fmt(salary,d.currency)}/yr</b></p><p><span>Estimated take-home</span><b>{fmt(r.takeHome,d.currency)}</b></p><p><span>Market rent</span><b>{fmt(d.rent,d.currency)}</b></p><p><span>Living costs · household {family}</span><b>{fmt(r.living,d.currency)}</b></p><p><span>Car payment</span><b>{fmt(car,d.currency)}</b></p><p><span>Debt payments</span><b>{fmt(debt,d.currency)}</b></p><p className="total"><span>Total monthly expenses</span><b>{fmt(r.total,d.currency)}</b></p><p><span>Money left</span><b>{fmt(r.left,d.currency)}</b></p><p><span>Money-left rate</span><b>{r.savingsRate.toFixed(1)}%</b></p></div><small className="source">Rent: {d.rentSource}. Updated {d.updated}.</small></article>}
